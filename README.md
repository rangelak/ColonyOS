...
2. Name it (e.g. "ColonyOS") and select your workspace.

### 2. Configure permissions and Socket Mode
...
**Socket Mode** — enable Socket Mode and generate an **App-Level Token** with the `connections:write` scope.

**Event Subscriptions** — subscribe to:
- `app_mention` (for mention-based triggers)
- `reaction_added` (for emoji-based triggers)
