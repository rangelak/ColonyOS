...
async def run_phase_interactive(
    phase: Phase,
    prompt: str,
    *,
    cwd: Path,
    ...
    When ``input_queue`` is provided, user messages are drained from the queue
    after each agent turn (AssistantMessage) and injected as follow-up queries.
    Falls back to standard ``run_phase()`` when ``input_queue`` is None.
    """
    ...
    if ui is None:
        _log(f"Starting {phase.value} phase [interactive] (budget=${budget_usd:.2f})...")
    ...
    try:
        async with ClaudeSDKClient(options) as client:
            ...
            while True:
                ...
                user_msg = _drain_input_queue(input_queue)
                if user_msg:
                    result_msg = None
                    wrapped = (
                        f"[USER DIRECTION]: {user_msg}\n\n"
                        "Continue your work, incorporating this feedback."
                    )
                    if on_user_input is not None:
                        on_user_input(user_msg)
                    if ui is not None:
        ...
        if ui is not None:
            ...
            _log(error_msg)
        logger.debug("Phase %s raw exception: %r", phase.value, exc)
        return PhaseResult(phase=phase, success=False, model=model, error=friendly)
    ...
    if result_msg is None:
        ...
        if ui is not None:
            ...
            _log(err)
        return PhaseResult(phase=phase, success=False, model=model, error=err)
...
def _drain_input_queue(q: _queue_mod.Queue[str]) -> str | None:
    """Non-blocking drain: concatenate all queued messages into one string."""
    parts: list[str] = []
    while True:
        try:
...
def run_phase_interactive_sync(
    phase: Phase,
    prompt: str,
    *,
    cwd: Path,
