...
class TestFixCompletionReactions:
    """Tests for the fix run completion reaction logic (task 3.0).

    The fix completion block in QueueExecutor._process_thread_fixes removes :eyes:,
    adds the status emoji, and adds :tada: on success only.
    Uses the same pattern as the main completion path.
    """

    def _simulate_fix_completion_reactions(self, client, channel, thread_ts, status):
        """Replicate the fix completion reaction block from cli.py."""
        from colonyos.slack import react_to_message, remove_reaction
        import logging
    ...
    def test_fix_success_removes_eyes_adds_check_and_tada(self):
        """On fix success: remove :eyes:, add :white_check_mark:, add :tada:."""
        client = MagicMock()
    ...
    def test_fix_failure_removes_eyes_adds_x_no_tada(self):
        """On fix failure: remove :eyes:, add :x:, no :tada:."""
        client = MagicMock()
        self._simulate_fix_completion_reactions(client, "C1", "ts1", RunStatus.FAILED)

        client.reactions_remove.assert_called_once_with(channel="C1", timestamp="ts1", name="eyes")
        client.reactions_add.assert_called_once_with(channel="C1", timestamp="ts1", name="x")

    def test_fix_eyes_removal_called_before_status_emoji(self):
        """remove_reaction('eyes') must be called before react_to_message(status_emoji) in fix path."""
        call_order = []
        client = MagicMock()
        client.reactions_remove.side_effect = lambda **kw: call_order.append(("remove", kw["name"]))
        client.reactions_add.side_effect = lambda **kw: call_order.append(("add", kw["name"]))
    ...
    def test_fix_eyes_removal_failure_does_not_block_status_emoji(self):
        """If removing :eyes: fails in fix path, the status emoji is still added."""
        client = MagicMock()
        client.reactions_remove.side_effect = Exception("no_reaction")
    ...
    def test_fix_tada_failure_does_not_raise(self):
        """If adding :tada: fails in fix path, no exception propagates."""
        client = MagicMock()
        # First reactions_add (white_check_mark) succeeds, second (tada) fails
        client.reactions_add.side_effect = [None, Exception("already_reacted")]
